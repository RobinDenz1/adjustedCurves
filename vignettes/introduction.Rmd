...
Doing this returns a list with some needed output objects. Most important however is the **adjsurv** object in there, containing the adjusted survival curves and corresponding confidence intervals (because **conf_int=TRUE** in the original function call). We can take a look at this object using the following code:
...
plot(adjsurv)
```
This plot function comes with many options which are listed in the documentation. To plot the pointwise confidence intervals, we can set the argument **draw_ci** to TRUE:
...
## Testing the Difference of Two Survival Curves
