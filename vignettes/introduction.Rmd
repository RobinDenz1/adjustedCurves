...
The function returns adjusted RMST for every level of the "group" variable. When bootstrapping was performed when calling the `adjustedsurv` function, standard deviations and confidence intervals can be calculated using this function as well by setting the `conf_int` argument to `TRUE`:
...
adjrmst <- adjusted_rmst(adjsurv, to=1, conf_int=TRUE)
print(adjrmst)
```
...
In some cases the researcher might be interested to formally test the hypothesis that two adjusted survival curves or two cumulative incidence functions are different in a certain interval. This can be done with the `adjusted_curve_test` function. It calculates the integral of the difference between two curves for each bootstrap estimate, shifts that distribution to be centered around 0 and calculates a p-value based on that shifted distribution:
...
The `adjusted_curve_test` function can also be used with more than two groups. In that case it returns pairwise comparisons.
