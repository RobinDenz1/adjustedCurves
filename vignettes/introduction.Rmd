...
We use `bootstrap=TRUE` here because we will need it in the next section. In practice, however, 150 bootstrap samples might not be enough to guarantee convergence.
...
There seems to be no statistically significant difference between the two RMSTs.
...
```{r echo=TRUE}
adjusted_rmtl(adjcif, to=0.4, conf_int=TRUE)
```
