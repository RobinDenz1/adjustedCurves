...
To calculate confounder-adjusted survival curves fo each group of this dataset using *Direct Standardization* (also known as *G-Computation* or *Corrected Group Prognosis method*), we first have to fit a **coxph** model:
...
```{r echo=TRUE, fig.show=TRUE, fig.width=5}
plot(adjsurv)
```
...
Doubly-Robust methods can be helpful in such cases (ZITATE). The standard Augmented Inverse Probability of Treatment Weighting estimator utilized both kinds of models at the same time. If either of the models is correctly specified, the resulting estimates will be unbiased. It can be used in the same way as the other methods, only that this time both models have to be supplied:
...
Another doubly-robust method is targeted maximum likelihood estimation (ZITATE). While the theory itself is a little more complicated, it can be used just as easily using the **adjustedCurves** package. Since our data is continuous, only the **tmle_pseudo** method can be used appropriately here (ZITATE). This can be done using the following **adjustedsurv** call.

```{r echo=TRUE, fig.width=5, message=FALSE, warning=FALSE}
...
In many situations there are multiple mutually exclusive types of events instead of just one event. This is formally known as a competing risks situation (ZITATE). In these situations, survival curves can not be estimated anymore (ZITAT). However, the cumulative incidence function can be used instead. Without randomization, these CIFs face the same problems due to confounding as the survival curves do. Most of the methods to adjust survival curves for confounders can be used to adjust CIFs in very similar fashion.

While the computational details and the underlying theory is slightly different (ZITATE), the syntax for the R-Package stays pretty much exactly the same. The only major difference is that instead of using the **adjustedsurv** function, the **adjustedcif** function should be utilized. Additionally, the user now also has to specify which event-type is of interest (argument "cause").
...
```{r echo=TRUE}
#TODO: HIER DIE ANDERE SIMULATIONSFUNKTION
data <- sim_confounded_surv(n=300)
data$group <- as.factor(data$group)
data$event[data$event==1] <- sample(c(1, 2), size=sum(data$event), replace=T)
```

### Direct Standardization

Let's again start with the *Direct Standardization* method. Instead of using a simple **coxph** method, we need to use a model for the time-to-event process which takes the multiple event-types into account. One such method is the *Cause-Specific Cox-Regression* model (ZITAT). A simple implementation of this model is contained in the **riskRegression** R-Package.
...
```{r echo=TRUE, message=FALSE, fig.show=TRUE, fig.width=5}
