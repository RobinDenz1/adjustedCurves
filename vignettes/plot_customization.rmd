...
knitr::opts_chunk$set(echo=TRUE, fig.width=6.5, fig.height=5)
```
...
plot(s_iptw, risk_table=TRUE, risk_table_stratify=TRUE,
     ...
     risk_table_title_size=10, risk_table_title_position="left",
...
The preceeding example could be fixed using the following code:
...
plot(s_iptw, conf_int=TRUE, censoring_ind="lines", risk_table=TRUE,
     ...
     xlab="Time in Years", risk_table_title_position="left")
```
