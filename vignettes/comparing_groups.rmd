...
```{r}
adjusted_surv_quantile(adjsurv, p=0.5, conf_int=TRUE, ratio=TRUE)
```
...
$$RMST_{diff}(to) = RMST_0(to) - RMST_1(to)$$

or the ratio between them:

$$RMST_{ratio}(to) = \frac{RMST_1(to)}{RMST_0(to)}.$$
...
```{r}
plot_rmst_curve(adjsurv, conf_int=TRUE, difference=TRUE)
```

or their ratio:

```{r}
plot_rmst_curve(adjsurv, conf_int=TRUE, ratio=TRUE)
```
